...
- **§5.11 win modal deviation.** The spec asked for a minimal `WAVE N CLEAR / Viability V% / Budget $B / NEXT WAVE →` panel. Shipped implementation reuses the existing `showWaveResultToast` DOM for visual continuity with Slice A — it's richer (current-round stats block, next-wave preview, continue button) but may feel busier than the spec intended. Playtest decision: if the toast-style reads as a win celebration, keep it; if not, add a Slice C task to build a minimal cyberpunk-HUD-native win panel.
- **`syncPaletteState` doesn't mirror `hidden`.** The cyberpunk palette cells sync `disabled` and `placing` from the classic buttons but not `hidden`. Wave 1 doesn't expose this because `WAVE_1.availableComponents === ["server", "database"]` lines up with the first two PALETTE entries, but from Wave 2+ the spec filters palette buttons by `wave.availableComponents` via `b.hidden = !allowed` in `td-mode.ts:202`. Under the current code, a wave-locked type's cyberpunk cell stays visible and clickable; a player could click it, dismiss the dossier, and then the `classicBtn?.click()` forward lands on a hidden classic button — a no-op with no visible feedback. One-line fix for Slice C: mirror `hidden` in `syncPaletteState` and add `"hidden"` to its MutationObserver's `attributeFilter`.
